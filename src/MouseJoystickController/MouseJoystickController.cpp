...
void MouseJoystickController::updateTorqueField() {
  // TODO: verify its behaviour
    ...
    // Calculate opposing force (proportional to velocity)
    float opposing_force = -torque_field_.damping_coefficient * velocity;
    
    // Convert force to current
    float current_scaling = 1.0f;  // mA/(unit force) - needs calibration
    ...
    // Set motor current only for the encoded axis
    setMotorTorque(PullAxisConstants::PULL_AXIS, desired_current);
    
    // Store encoder position for next iteration
